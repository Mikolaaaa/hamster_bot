...
from db import init_db, get_user, add_user, update_coins, SessionLocal
import random
...
async def show_main_menu(chat_id: int, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        button3 = types.InlineKeyboardButton(text="📊 Статистика", callback_data='stats')
        button4 = types.InlineKeyboardButton(text="🎲 Игра: Орел/Решка", callback_data='coinflip')
        button5 = types.InlineKeyboardButton(text="🔢 Игра: Угадай число", callback_data='guess_number')
...
async def process_buy_hamster_level(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        if user.coins >= 10:  # Проверяем, достаточно ли монет
            user.coins -= 10
...
async def process_shop(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        button1 = types.InlineKeyboardButton(text="🏅 Купить уровень множителя за 10 монет", callback_data='buy_multiplier')
        button2 = types.InlineKeyboardButton(text="🥇 Поднять уровень хомяка за 10 монет", callback_data='buy_hamster_level')
...
if __name__ == '__main__':
    # Запуск бота с передачей объекта бота в start_polling
